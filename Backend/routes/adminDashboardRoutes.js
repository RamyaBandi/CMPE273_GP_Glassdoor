...
if (process.env.KAFKA_SWITCH === 'true') {
    adminAnalyticsRouter.route(GET_REVIEWS_PER_DAY).get(checkAuth, adminAnalyticsKafkaServices.reviewsperday);

    adminAnalyticsRouter.route(GET_TOP_REVIEWED_COMPANIES).get(checkAuth, adminAnalyticsKafkaServices.topreviewedcompanies);

    adminAnalyticsRouter.route(GET_TOP_AVERAGE_RATED_COMPANIES).get(checkAuth, adminAnalyticsKafkaServices.averageratedcompanies);

    adminAnalyticsRouter.route(GET_TOP_STUDENTS_ON_RATING).get(checkAuth, adminAnalyticsKafkaServices.topstudentratings);

    adminAnalyticsRouter.route(GET_TOP_CEO_RATING).get(checkAuth, adminAnalyticsKafkaServices.topceorating);

    adminAnalyticsRouter.route(GET_TOP_COMPANY_VIEWS).get(checkAuth, adminAnalyticsKafkaServices.topcompanyviews);
}
else {
