...
export default class JobApplication extends Component {
  ...
  componentDidMount = async () => {
    ...
    await axios
      .get(
        BACKEND_URL + JOB_ROUTE + GET_COMPANY_JOB_BY_JOBID + "?jobId=" + jobId
      )
      .then((response) => {
        ...
        axios
          .get(BACKEND_URL + GET_COMPANY_DETAILS + "?companyId=" + companyId)
          .then((response) => {
            this.setState({
  ...
  apply = (e) => {
    ...
    axios
      .post(
        `${BACKEND_URL}${APPLICATION_ROUTE}${POST_APPLICATION}?studentId=${data.studentId}&jobId=${data.jobId}&resumeUploaded=${data.resumeUploaded}&coverLetterUploaded=${data.coverLetterUploaded}`,
        mediaForm
      )
      .then((response) => {
        //redirect
  ...
  render = () => {
    ...
    let renderVar
    if(this.state.jobDetails && this.state.companyDetails)
    {
        renderVar=<div
        style={{
      ...
      >
        ...
        >
          <Row>
            <Col md="6">
              <Row>
                <Col md="12">{this.state.companyDetails.companyName}</Col>
              </Row>
              <Row>
                <Col md="12">
                  ...
                  <Row>
                    <Col md="12">
                      <div style={{ marginTop: "10px", textAlign: "center" }}>
                        {this.state.resumeProgress === 0 ? (
                          <b style={{ color: "red" }}>No Resume!</b>
                        ) : this.state.resumeProgress === 100 ? (
                          <b style={{ color: "green" }}>Success!</b>
                        ) : (
                          <ProgressBar
                            striped
                            now={this.state.resumeProgress}
                            label={`${this.state.resumeProgress}%`}
                            variant="warning"
                          />
                        )}
                      </div>
                ...
                <Col md="auto">
                  <Row>
                    <Col md="12">
                      <>
                        <Button
                          onClick={this.handleClickCoverLetter}
                          variant="success"
                        >
                          Upload Cover Letter
                        </Button>
                  ...
                  <Row>
                    <Col md="12">
                      <div style={{ marginTop: "10px", textAlign: "center" }}>
                        {this.state.coverLetterProgress === 0 ? (
                          <b style={{ color: "red" }}>No Cover Letter!</b>
                        ) : this.state.coverLetterProgress === 100 ? (
                          <b style={{ color: "green" }}>Success!</b>
                        ) : (
                          <ProgressBar
                            striped
                            now={this.state.coverLetterProgress}
                            label={`${this.state.coverLetterProgress}%`}
                            variant="warning"
                          />
                        )}
                      </div>
          ...
          <Row>
            <Col md="12">
              <Row>
                ...
                <Col md="auto">
                  <p>{this.state.jobDetails.postedDate}</p>
                </Col>
              </Row>
            </Col>
            <Col md="12">
              <Row>
                ...
                <Col md="auto">
                  {" "}
                  {this.state.jobDetails.remote ? (
                    <p>Remote</p>
                  ) : (
                    <p>On Site at the Office Address.</p>
                  )}
                </Col>
              </Row>
            </Col>
              ...
              <Row>
                ...
                <Col md="auto">
                  <Row style={{ height: "15px" }}>
                    <Col>{this.state.jobDetails.streetAddress}</Col>
                  </Row>
                  <Row>
                    <Col>
                      {this.state.jobDetails.city},{" "}
                  ...
                  <Row>
                    <Col>
                      {this.state.jobDetails.country},{" "}
                      {this.state.jobDetails.zip}.
                    </Col>
                  </Row>
                </Col>
              </Row>
            ...
            <Col md="12" style={{ marginTop: "10px" }}>
              <Row>
                ...
                <Col md="auto">
                  {" "}
                  <p>{this.state.jobDetails.responsibilities}</p>
                </Col>
              </Row>
            </Col>
          </Row>
